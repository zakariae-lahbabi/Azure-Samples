...
class ShoppingCartUser(HttpUser):
    ...
    def get_by_search(self):
        term = fake.lexify("????")
        self.client.get(
            f"/shopping_cart/search/{term}", name="/shopping_cart/search/{id}"
        )
    ...
    def get_by_package_id(self):
        id = fake.random_int(1, 10000)
        self.client.get(
            f"/shopping_cart/package/{id}", name="/shopping_cart/package/{id}"
        )
        ...
        for _ in range(fake.random_int(1, 10)):
            ...
            for i in range(fake.random_int(1, 10)):
                ...
                if i >= 7:
                    ...
                    item["details"]["package"]["id"] = fake.random_int(
                        1, 10000)
